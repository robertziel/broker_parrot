...
def test_set_and_get_round_trip():
    worker_control.set_worker_control(
        "box-b", "gpu", desired_state="off", stop_policy="hard",
        requested_by="op@example",
    )
    row = worker_control.get_worker_control("box-b", "gpu")
    assert row is not None
    assert row["host_label"] == "box-b"
...
def test_desired_state_for_reads_off_and_on():
    worker_control.disable_worker("box-a", "gpu")
    assert worker_control.desired_state_for("box-a", "gpu") == "off"
    worker_control.enable_worker("box-a", "gpu")
    assert worker_control.desired_state_for("box-a", "gpu") == "on"


def test_enable_disable_helpers():
    worker_control.disable_worker("box-a2", "gpu", requested_by="me")
    r = worker_control.get_worker_control("box-a2", "gpu")
    assert r["desired_state"] == "off" and r["stop_policy"] == "hard"
    worker_control.enable_worker("box-a2", "gpu")
    assert worker_control.get_worker_control("box-a2", "gpu")["desired_state"] == "on"
...
def test_set_worker_control_fires_notify():
    ...
    with psycopg.connect(db_url(), autocommit=True) as conn:
        ...
        worker_control.set_worker_control("box-b", "gpu", desired_state="off")
        payloads = [n.payload for n in conn.notifies(timeout=3.0, stop_after=1)]
    assert payloads == ["box-b:gpu"]
...
def test_requeue_running_for_worker_node():
    job_id = _running_node_job("box-a", queue="gpu")
    n = node_queue.requeue_running_for_worker("box-a", "gpu")
    assert n == 1
...
def test_requeue_does_not_increment_watchdog_retries():
    ...
    job_id = _running_node_job("box-a", queue="gpu")
    node_queue.requeue_running_for_worker("box-a", "gpu")
    assert (node_queue.get_node_job(job_id).get("watchdog_retries") or 0) == 0


def test_requeue_scoped_to_host():
    mine = _running_node_job("box-a", queue="gpu")
    theirs = _running_node_job("box-b", queue="gpu")
    assert node_queue.requeue_running_for_worker("box-a", "gpu") == 1
...
def test_requeue_scoped_to_queue():
    """box-b runs a cpu AND a gpu worker under one host_label: turning OFF the
    gpu worker must not release the cpu worker's in-flight job."""
    gpu_job = _running_node_job("box-b", queue="gpu")
    cpu_job = _running_node_job("box-b", queue="cpu")
    assert node_queue.requeue_running_for_worker("box-b", "gpu") == 1
...
def test_requeue_targets_ingest_table_for_ingest_queue():
    job_id = _running_ingest_job("box-b", queue="fetch")
    n = node_queue.requeue_running_for_worker("box-b", "fetch")
    assert n == 1
