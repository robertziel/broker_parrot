...
def test_check_once_hard_stops_when_off():
    worker = _worker("box-a")
    job_id = _running_node_job("box-a", queue="cpu")
    worker_control.disable_worker("box-a", "cpu")  # OFF, hard
...
def test_check_once_noop_when_on_or_absent():
    ...
    worker_control.enable_worker("box-a", "cpu")
...
def test_check_once_scoped_to_this_workers_queue():
    ...
    turned off."""
    worker = _worker("box-b", queue="cpu")
    worker_control.disable_worker("box-b", "gpu")  # gpu off, cpu still on
    exits: list[int] = []
    w = worker_control.WorkerControlWatcher(
        worker=worker, on_exit=lambda code: exits.append(code),
    )
