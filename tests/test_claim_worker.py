...
def test_job_status_watcher_kills_when_job_requeued():
    ...
    node_queue.claim_next_gpu_job(0, host="spark")  # running, claimed_by=spark

    exits: list[int] = []
    w = claim_worker.JobStatusWatcher(
        job_id=job_id, claimed_by="spark",
...
def test_job_status_watcher_quiet_while_job_ours_and_running():
    ...
    node_queue.claim_next_gpu_job(0, host="spark")
...
def test_run_node_threads_a_callable_status_callback_to_gpu_node():
    ...
    worker = claim_worker.ClaimWorker(queue="gpu", host="spark2", model_cache=_Cache())
    assert worker.run_once() is True
...
def test_stall_watchdog_not_armed_for_video_models(monkeypatch):
    ...
    worker = claim_worker.ClaimWorker(queue="gpu", host="spark", model_cache=_Cache())
    assert worker.run_once() is True
...
def test_gpu_job_gets_health_watchdog_and_no_wallclock_watchdog(monkeypatch):
    ...
    assert claim_worker.ClaimWorker(queue="cpu", host="beelink").run_once() is True
...
def test_claim_worker_accepts_fetch_and_load_queues():
    for q in ("fetch", "load"):
        w = claim_worker.ClaimWorker(queue=q, host="beelink")
        assert w.queue == q
...
def test_run_once_claims_and_executes_ingest_job():
    ...
    worker = claim_worker.ClaimWorker(queue="fetch", host="beelink")
    assert worker.run_once() is True
    assert ran == ["tick"]
    row = node_queue.get_ingest_job(job_id)
    assert row["status"] == "completed"
    assert row["claimed_by"] == "beelink"


def test_run_once_ingest_returns_false_when_empty():
    worker = claim_worker.ClaimWorker(queue="load", host="beelink")
    assert worker.run_once() is False
...
def test_heartbeat_emit_once_upserts_cpu_row(_heartbeat_enabled):
    emitter = claim_worker.HeartbeatEmitter(queue="cpu", host_label="beelink")
    emitter.emit_once()
    row = _heartbeat_row("beelink", "cpu")
...
def test_heartbeat_emit_once_gpu_reports_current_model(_heartbeat_enabled):
    ...
    emitter = claim_worker.HeartbeatEmitter(
        queue="gpu", host_label="spark", model_cache=cache,
    )
    ...
    emitter.emit_once()
    assert _heartbeat_row("spark", "gpu")["current_model"] == "qwen_edit"
...
def test_heartbeat_disabled_by_env(monkeypatch):
    ...
    assert _heartbeat_row("beelink", "cpu") is None


def test_heartbeat_thread_refreshes_then_stops(_heartbeat_enabled):
    emitter = claim_worker.HeartbeatEmitter(
        queue="cpu", host_label="beelink", interval_s=0.02,
    )
    emitter.start()
    first = _heartbeat_row("beelink", "cpu")
    assert first is not None

    with connection() as c, c.cursor() as cur:
        cur.execute(
            "SELECT last_seen FROM worker_heartbeats "
            "WHERE host_label='beelink' AND queue='cpu'"
        )
...
def test_claim_worker_wires_heartbeat_emitter():
    ...
    w = claim_worker.ClaimWorker(queue="gpu", host="spark", model_cache=_Cache())
    assert w.heartbeat._queue == "gpu"
    assert w.heartbeat._host_label == "spark"
    assert w.heartbeat._current_model() == "sdxl"

    cpu = claim_worker.ClaimWorker(queue="cpu", host="beelink")
