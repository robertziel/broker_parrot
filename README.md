...
queue-worker-control --queue gpu --on  --host box-a   # turn it back on
```
...
worker_control.enable_worker("box-a", "gpu")      # resume in place (no restart)
worker_control.desired_state_for("box-a", "gpu")  # 'on' | 'off'  (absent ⇒ 'on')
```
