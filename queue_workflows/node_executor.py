...
def _invoke(
    ...
    for name, param in sig.parameters.items():
        ...
        if name == "out":
            ...
            # The LLM server (type + address + port) broker_parrot resolved for
            # THIS job at dispatch — a node that declares ``llm_server`` calls the
            # handed-in endpoint instead of hardcoding a URL. None when no host
            # resolver is wired (see config.resolve_llm_server / execute_node).
            kwargs["llm_server"] = llm_server
...
def execute_node(
    ...
    *,
    ...
    with _invoke_cm as _finalize:
        try:
            # Resolve the LLM server (type+address+port) for THIS job at dispatch
            # and hand it to the node — parallel to model_handle. None unless a
            # host wired set_llm_server_resolver. Inside the try so a resolver
            # error fails the node honestly (same as any invoke-time failure).
            llm_server = cfg.resolve_llm_server(job)
            result = _invoke(
