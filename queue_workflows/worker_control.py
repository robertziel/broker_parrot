...
def _project(project: str | None = None) -> str:
    """The tenant this control row belongs to (migration 0019).

    ``None`` ⇒ this process's ``config.project``, which is exactly the tenant of
    the worker asking — so the claim worker's park-gate and the backend factory
    stay project-correct with no call-site change. Single-tenant deploys leave
    ``config.project`` unset ⇒ ``''``, the pre-0019 sentinel."""
...
def set_worker_control(
    ...
    *,
    ...
    """Upsert the desired control state for a ``(host_label, queue, project)``
    worker.

    Project-scoped (migration 0019): ``project`` defaults to ``config.project``.
    On a shared broker ``host_label`` is not globally unique — two projects can
    run a worker on the same machine + queue — so without the tenant term an
    operator OFF for project A would also hard-stop project B's worker there.
    ...
    """
    params = (
        host_label, queue, _project(project), desired_state, stop_policy, requested_by,
    )
    if conn is not None:
...
def get_worker_control(
    ...
    * ``UndefinedColumn`` (0012 applied, 0019 not) ⇒ retry the pre-0019 2-col
    ...
    try:
        ...
        pass  # pre-0019 DB — fall through to the 2-col lookup.
    try:
        with connection() as conn, conn.cursor() as cur:
            cur.execute(
                f"{cols} FROM worker_controls "
...
def desired_state_for(
    ...
    row = get_worker_control(host_label, queue, project=project)
...
def disable_worker(
    ...
    requested_by: str | None = None, project: str | None = None,
) -> None:
    ...
    set_worker_control(
        ...
        stop_policy=stop_policy, requested_by=requested_by, project=project,
    )
...
def set_llm_config(
    ...
    *,
    ...
    """Upsert the LLM server config for a ``(host_label, queue, project)`` worker.

    Project-scoped (migration 0019); ``project`` defaults to ``config.project``.
    Without the tenant term, setting project A's box to ``vllm`` would flip
    project B's worker on the same machine + queue to ``vllm`` too — silently
    reconfiguring another tenant's LLM server.
    ...
    sql = """
        INSERT INTO worker_controls
            (host_label, queue, project, llm_server_type, llm_parallelism,
             vllm_idle_ttl_s, updated_at)
        VALUES (
        ...
        ON CONFLICT (host_label, queue, project) DO UPDATE SET
...
def llm_config_for(
    ...
    try:
        ...
        # the operator's answer. Try it; if THAT also raises, take the defaults.
        try:
            with connection() as conn, conn.cursor() as cur:
                cur.execute(
...
def main(argv: list[str] | None = None) -> int:
    ...
    ``worker_controls`` row for a ``(host, queue, project)`` worker; the row
    trigger wakes the worker. Defaults ``--host`` to this box's label and
    ``--project`` to ``config.project``.

    On a shared broker ``--project`` is what keeps an OFF from parking another
    tenant's worker on the same machine + queue."""
    import argparse
    ...
    parser.add_argument(
        ...
        help="tenant whose worker to control on a shared broker "
             "(default: this process's configured project)",
    )
