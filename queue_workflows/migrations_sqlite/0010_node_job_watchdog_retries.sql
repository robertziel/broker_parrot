...
--
-- Additive + idempotent (``ADD COLUMN`` with a NOT NULL DEFAULT 0,
-- which SQLite backfills existing rows to 0 without a table rewrite) so
-- re-running on the live database is a safe no-op. No index: the column is
