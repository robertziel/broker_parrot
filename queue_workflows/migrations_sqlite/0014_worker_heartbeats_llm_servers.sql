...
-- ONLY on the NVIDIA box-c hubs, never on the AMD/ROCm control box (host-c). The
...
--
-- SQLite: text[] → TEXT, IF NOT EXISTS on ADD COLUMN not supported.

ALTER TABLE worker_heartbeats
    ADD COLUMN llm_servers_available TEXT NOT NULL DEFAULT '["ollama"]';
