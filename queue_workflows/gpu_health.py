...
shares the GB10 GPU on spark/spark2 (the box-level util would false-negative a
real render hang because ollama keeps the box GPU > 0 %).


GPU utilization — ``gpu_util_pct()``
------------------------------------
NVIDIA (GB10/Blackwell): ``nvidia-smi pmon -c 1 -s u`` reports a per-PROCESS
...
not listed (verified on spark2: ollama's busy python is invisible from the
render container's pmon, and vice-versa). We take the MAX ``sm%`` across the
returned rows as the per-container GPU-busy signal.

LIMITATION (documented, by design): on the GB10 the per-PID ``sm%`` column is
...
ROCm (beelink overflow worker) + any non-NVIDIA / no-pmon box: pmon isn't
available, so ``gpu_util_pct()`` falls back to the box-level
``nvidia-smi``/``rocm-smi`` utilization (via ``hw_metrics._gpu_probe``). On
beelink there is no ollama GPU sidecar, so the box-level signal is already
per-host-clean there; and beelink runs the qwen pipeline fine (the stall is
GB10-specific), so the fallback's coarser attribution is acceptable for the
overflow host.
...
# pmon single-sample call is cheap (~0.06 s on GB10); cap it hard so a stuck
...
def _nvidia_pmon_container_sm_pct() -> int | None:
    ...
    common GB10 case for a process not actively issuing SM work) counts as 0.
    """
    try:
...
def _box_gpu_util_pct() -> int:
    ...
    Fallback for ROCm / no-pmon hosts. NOTE: this is BOX-level — on a GB10 with
    ...
    preferred. Used only where the per-container path is unavailable (beelink
